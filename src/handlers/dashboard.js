...
export function registerDashboardHandlers(bot, config) {
  ...
  bot.action('feature_ads_logs', async ctx => {
    await ctx.answerCbQuery();
    const ads = await AdCampaign.find({ ownerId: ctx.from.id }).sort({ createdAt: -1 }).limit(20).lean();
    const lines = ads.length
      ? ads.map((a,i) => (i+1) + '. ' + a.title + ' · ' + a.status + ' · ₹' + a.price + ' · ' + a.successful + '/' + a.targetCount).join('\n')
      : 'No ad campaigns yet.';
    await edit(ctx, '📋 <b>ADS LOGS</b>\n\n' + lines, simpleBackKeyboard());
  });
  ...
  bot.on('text', async (ctx, next) => {
    ...
    if (!text) return ctx.reply('❌ Please send a valid value.');
    if (text === '/cancel') {
    ...
    if (state.data.action === 'template_name') {
      await setUiState(ctx.from.id, stateKey, { action: 'template_text', name: text.slice(0, 100) });
      await ctx.reply('✏️ Now send the message text for <b>' + text.replace(/[<&>]/g, '') + '</b>.', { parse_mode: 'HTML' });
      return;
    }

    if (state.data.action === 'template_text') {
      const name = state.data.name;
      const existing = await MessageTemplate.findOne({ ownerId: ctx.from.id, name });
      const template = existing || new MessageTemplate({ ownerId: ctx.from.id, name });
      template.text = text.slice(0, 4096);
      template.type = 'text';
      await template.save();
      await clearUiState(ctx.from.id, stateKey);
      await ctx.reply('✅ Message template saved: ' + name, mainKeyboard());
      return;
    }
    ...
    if (state.data.action === 'template_edit') {
      ...
      if (!template) {
        await clearUiState(ctx.from.id, stateKey);
        return ctx.reply('❌ Message template not found.', mainKeyboard());
      }
      template.text = text.slice(0, 4096);
      await template.save();
      await clearUiState(ctx.from.id, stateKey);
      await ctx.reply('✅ Message updated.', mainKeyboard());
      return;
    }

    if (state.data.action === 'redeem_code') {
      await clearUiState(ctx.from.id, stateKey);
      return ctx.reply('🎁 Redeem system is reserved for the wallet phase. Code received: ' + text);
    }
